...
//                new FastBasicKeypointMatcher<Keypoint>(8));
        final RobustHomographyEstimator ransac = new RobustHomographyEstimator(0.5, 500,
...
//        this.matcher5.setFittingModel(ransac);
    ...
    public void StartVideo() throws IOException {
        ...
        String o1 = "output/features1.txt";
        ...
        for (MBFImage mbfImage : video) {
           ...
           // renderer.drawPoints(kpl, RGBColour.MAGENTA, 3);
                System.out.println(((MatrixTransformProvider) this.matcher1.getModel()).getTransform().cond());
                System.out.println(this.matcher1.findMatches(kpl));
            if (this.matcher1.findMatches(kpl)
                ...
                try {
                    ...
                    if (modelImage1.getBounds().transform(boundsToPoly).isConvex()) {
                        ...
                        renderer.drawText("Johnny", ((int) (this.modelImage1.getBounds().transform(boundsToPoly).minX() +  this.modelImage1.getBounds().transform(boundsToPoly).maxX())/2), ((int) (this.modelImage1.getBounds().transform(boundsToPoly).minY() +  this.modelImage1.getBounds().transform(boundsToPoly).maxY())/2), HersheyFont.ASTROLOGY, 20, RGBColour.BLUE);
            ...
            if (this.matcher2.findMatches(kpl)
                ...
                try {
                    ...
                    renderer.drawText("Rachel", ((int) (this.modelImage2.getBounds().transform(boundsToPoly).minX() +  this.modelImage2.getBounds().transform(boundsToPoly).maxX())/2), ((int) (this.modelImage2.getBounds().transform(boundsToPoly).minY() +  this.modelImage2.getBounds().transform(boundsToPoly).maxY())/2), HersheyFont.ASTROLOGY, 20, RGBColour.BLUE);
...
//                    && ((MatrixTransformProvider) this.matcher4.getModel()).getTransform().cond() < 1e9) {
//                try {
...
//                    && ((MatrixTransformProvider) this.matcher5.getModel()).getTransform().cond() < 1e9) {
//                try {
...
//            }
    ...
    public void LoadReferenceObject() {
        ...
        try {
            modelImage1 = ImageUtilities.readMBF(new File("data/Johnny.jpg"));
            modelImage2 = ImageUtilities.readMBF(new File("data/Rachel.JPG"));
//            modelImage3 = ImageUtilities.readMBF(new File("data/Surabhi1.jpg"));
//            modelImage4 = ImageUtilities.readMBF(new File("data/Surabhi2.jpg"));
//            modelImage5 = ImageUtilities.readMBF(new File("data/ChaiBisket.jpg"));
        } catch (IOException e) {
            e.printStackTrace();
        }
